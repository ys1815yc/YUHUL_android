...
public class STFT {
    ...
    protected double[][] spectrumAmpOutArray; // brandon modify
    ...
    final public double[] getSpectrumAmpInTmpValue(){
        double[] tmpSpectrumAmpInTmp = new double[spectrumAmpInTmp.length];
        System.arraycopy(spectrumAmpInTmp,0,tmpSpectrumAmpInTmp,0,spectrumAmpInTmp.length);
        return tmpSpectrumAmpInTmp;
    }
    public void feedBData(double[] ds){feedBData(ds,ds.length);}
    public void feedBData(double[] ds, int dsLen) {
        spectrumAmpFFT.bt(ds);
    }
